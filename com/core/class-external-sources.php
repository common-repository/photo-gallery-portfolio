...
class AdamLabsGallery_Facebook {
	/**
	 * Stream Array
	 */
	private $stream;
	...
	public function get_photo_set_photos($photo_set_id,$item_count=10,$app_id,$app_secret){
		...
		$transient_name = 'adamlabsgallery_' . md5($url."&limit=".$item_count);
			...
			if($item_count<=25){
				...
				if(!isset($photo_set_photos->error->message)){
					...
					if(empty($error_message)){
						$error_message = __("Facebook reports: ",ADAMLABS_GALLERY_TEXTDOMAIN).$photo_set_photos->error->message;
						echo $error_message;
						return false;
					}
				}
				...
				for ($i=0; $i < $runs && sizeof($this->stream) < $original_count && $supervisor_count < 20 ; $i++) {
					...
					if(!isset($page_rsp->error->message)){
						...
						if(empty($error_message)){
							$error_message = __("Facebook reports: ",ADAMLABS_GALLERY_TEXTDOMAIN).$page_rsp->error->message;
							echo $error_message;
							return false;
						}
					}
	...
	public function get_photo_feed($user,$app_id,$app_secret,$item_count=10){
		...
		if(!isset($oauth->access_token)) {
			$error_message = __("Please adjust the grid's Facebook credentials", ADAMLABS_GALLERY_TEXTDOMAIN);
			echo $error_message;
			return false;
		}
		...
		$transient_name = 'adamlabsgallery' . md5($url."&limit=".$item_count);
		...
		if ($this->transient_sec > 0 && false !== ($data = get_transient( $transient_name)))
			...
			if($item_count<=25){
				...
				if(!isset($feed->error->message)){
					...
					if(empty($error_message)){
						$error_message = __("Facebook reports: ",ADAMLABS_GALLERY_TEXTDOMAIN).$feed->error->message;
						echo $error_message;
						return false;
					}
				}
	...
	private function facebook_output_array($photos,$item_count){
		foreach ($photos as $photo) {
			...
			if(!empty($photo->comments->data))
				...
				$stream['likes_short'] = AdamLabsGallery_Base::thousandsViewFormat($photo->likes->summary->total_count);
				$this->stream[] = $stream;
		}
		return $item_count;
	}
...
class AdamLabsGallery_Twitter {
	...
	public function get_public_photos($twitter_account,$include_rts,$exclude_replies,$count,$imageonly){
		...
		$twitter_api = new AdamLabsGalleryTwitterApi( $credentials , $this->transient_sec);
	...
	private function twitter_output_array($tweets,$count,$imageonly){
		if(is_array($tweets)){
		  ...
		  $stream['retweets_short'] = AdamLabsGallery_Base::thousandsViewFormat($tweet->retweet_count);
		  $stream['likes'] = $tweet->favorite_count;
		  $stream['likes_short'] = AdamLabsGallery_Base::thousandsViewFormat($tweet->favorite_count);
 ...
 */
class AdamLabsGalleryTwitterApi {
...
class AdamLabsGallery_Instagram {
	...
	public function get_public_photos($search_user_id,$count,$orig_image){
		...
		if(!empty($search_user_id)){
			...
			$transient_name = 'adamlabsgallery_'. md5($cacheKey);
			if ($this->transient_sec > 0 && false !== ($data = get_transient( $transient_name))){
				$this->stream = $data;
				return $this->stream;
			}
				...
				if(isset($rsp->edge_owner_to_timeline_media))
					...
					if(!empty($this->stream)){
						...
						_e('Instagram reports: Please check the settings',ADAMLABS_GALLERY_TEXTDOMAIN);
						return false;
					}
		}
		else {
	...
	public function get_tag_photos($search_user_id,$count,$orig_image){
		if(!empty($search_user_id)){
			...
			$transient_name = 'adamlabsgallery_'. md5($url."count=".$count);
	...
	private function instagram_output_array($photos,$count,$search_user_id,$orig_image){
		foreach ($photos as $photo) {
			if($count > 0){
				...
				$stream['likes_short'] = AdamLabsGallery_Base::thousandsViewFormat($photo->edge_media_preview_like->count);
	...
	private function instagram_output_array_places($photos,$count,$search_user_id,$orig_image){
		foreach ($photos as $photo) {
			if($count > 0){
				...
				$stream['likes_short'] = AdamLabsGallery_Base::thousandsViewFormat($photo->likes->count);
		...
		switch ($page_res['http_code']) {
			...
			case 200:
				...
				if (!preg_match('#window\._sharedData\s*=\s*(.*?)\s*;\s*</script>#', $page_res['body'], $page_data_matches)) {
					_e('Instagram reports: Parse script error',ADAMLABS_GALLERY_TEXTDOMAIN);
		
				} else {
					$page_data = json_decode($page_data_matches[1], true);
		
					if (!$page_data || empty($page_data['entry_data']['ProfilePage'][0]['graphql']['user'])) {
						_e('Instagram reports: Content did not match expected',ADAMLABS_GALLERY_TEXTDOMAIN);
		
					} else {
						$user_data = $page_data['entry_data']['ProfilePage'][0]['graphql']['user'];
		
						if ($user_data['is_private']) {
							_e('Instagram reports: Content is private',ADAMLABS_GALLERY_TEXTDOMAIN);
...
class AdamLabsGallery_Flickr {
		...
		if ($this->transient_sec > 0 && false !== ($data = get_transient( $transient_name)))
			...
			if(isset($rsp->stat) && $rsp->stat == "fail"){
				if(empty($this->flickr_message)){
					$this->flickr_message = __('flickr reports: ', ADAMLABS_GALLERY_TEXTDOMAIN).$rsp->message ;
	...
	private function flickr_output_array($photos){
		foreach ($photos as $photo) {
			...
			$stream['views_short'] = AdamLabsGallery_Base::thousandsViewFormat($photo->views);
			...
			if(!empty($photo_favorites->photo->total)){
				...
				$stream['favorites_short'] = AdamLabsGallery_Base::thousandsViewFormat($photo_favorites->photo->total);
			}
...
class AdamLabsGallery_Youtube {
	...
	public function __construct($api_key,$channel_id,$transient_sec=86400) {
		...
		$this->transient_sec = $transient_sec;
		$this->enable_youtube_nocookie = get_option('adamlabsgallery_enable_youtube_nocookie', 'false');
	}
			...
			if($count<=$maxResults){
				...
				for ($i=0; $i < $runs && sizeof($this->stream) < $original_count && $supervisor_count < 20; $i++) {
					...
					if(!empty($page_rsp) && !isset($page_rsp->error->message) ){
						...
						echo __("YouTube reports: ",ADAMLABS_GALLERY_TEXTDOMAIN).$page_rsp->error->message;
						return false;
					}
				}
			}
	...
	private function youtube_playlist_output_array($videos,$count){
		...
		foreach ($videos as $video) {
			...
			if($count > 0){
				...
				$stream['views_short'] = AdamLabsGallery_Base::thousandsViewFormat($video_stats->items[0]->statistics->viewCount);
				$stream["likes"] = $video_stats->items[0]->statistics->likeCount;
				$stream["likes_short"] = AdamLabsGallery_Base::thousandsViewFormat($video_stats->items[0]->statistics->likeCount);
				$stream["dislikes"] = $video_stats->items[0]->statistics->dislikeCount;
				$stream["dislikes_short"] = AdamLabsGallery_Base::thousandsViewFormat($video_stats->items[0]->statistics->dislikeCount);
				$stream["favorites"] = $video_stats->items[0]->statistics->favoriteCount;
				$stream["favorites_short"] = AdamLabsGallery_Base::thousandsViewFormat($video_stats->items[0]->statistics->favoriteCount);
				$stream["num_comments"] = $video_stats->items[0]->statistics->commentCount;
				$stream["num_comments_short"] = AdamLabsGallery_Base::thousandsViewFormat($video_stats->items[0]->statistics->commentCount);
...
class AdamLabsGallery_Vimeo {
	...
	public function get_vimeo_videos($type,$value,$count){
		...
		$transient_name = 'adamlabsgallery_' . md5($url);
			...
			if($count > 20){
				...
				for ($i=0; $i < $runs && $supervisor_count < 20; $i++) {
					...
					if(!empty($page_rsp)){
						...
						if($i==0){
							_e("Vimeo reports: Please check your Data!", ADAMLABS_GALLERY_TEXTDOMAIN);
							return false;
						}
					}
				}
	...
	private function vimeo_output_array($videos,$count){
		if(is_array($videos))
			foreach ($videos as $video) {
				...
				$stream["likes_short"] = isset($video->stats_number_of_likes)? AdamLabsGallery_Base::thousandsViewFormat($video->stats_number_of_likes) : 0 ;
				$stream["views"] = isset($video->stats_number_of_plays)? $video->stats_number_of_plays : 0 ;
				$stream["views_short"] = isset($video->stats_number_of_plays)? AdamLabsGallery_Base::thousandsViewFormat($video->stats_number_of_plays) : 0 ;
...
class AdamLabsGallery_Rml {
	...
	public static function option_list_image_sizes($selected=""){
		$image_sizes = AdamLabsGallery_Rml::get_image_sizes();
		$options = "";
...
class AdamLabsGallery_Dribbble {
	/**
	 * Stream Array
	 */
	private $stream;
