...
class AdamLabsGallery_Wpml{
	...
	private static function validate_wpml_exists(){
		if(!self::is_wpml_exists())
			AdamLabsGallery_Base::throw_error(__("The wpml plugin don't exists", ADAMLABS_GALLERY_TEXTDOMAIN));
	}
	...
	public static function get_current_lang(){
		...
		return(apply_filters('adamlabsgallery_wpml_get_current_lang', $lang));
	}
	...
	public static function get_current_lang_code(){
		...
		return(apply_filters('adamlabsgallery_wpml_get_current_lang_code', $langTag));
	}
	...
	public static function get_id_from_lang_id($id, $type = 'category'){
		...
		return apply_filters('adamlabsgallery_wpml_get_id_from_lang_id', $mid, $id, $type);
	}
	...
	public static function get_lang_id_from_id($id, $type = 'category'){
		...
		return apply_filters('adamlabsgallery_wpml_get_lang_id_from_id', $mid, $id, $type);
	...
	public static function change_cat_id_by_lang($catID, $type = 'category'){
		...
		return apply_filters('adamlabsgallery_wpml_change_cat_id_by_lang', $mid, $catID, $type);
	...
	public static function strip_category_additions($text){
		...
		return apply_filters('adamlabsgallery_wpml_strip_category_additions', $mid, $text);
	}
