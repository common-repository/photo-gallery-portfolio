...
class AdamLabsGallery_Item_Skin {
    ...
    public function init_by_data($data){
		...
		$data = apply_filters('adamlabsgallery_init_by_data_item_skin', $data);
    ...
    public function init_by_id($id){
		...
		if(!empty($skin)){
			...
			$params = AdamLabsGallery_Base::stripslashes_deep(@json_decode($skin['params'], true));
			$this->params = $params;
			$layers = @json_decode($skin['layers'], true);
			if(!empty($layers) && is_array($layers)){ //prevent overhead
				foreach($layers as $lkey => $layer){
					$layers[$lkey] = AdamLabsGallery_Base::stripslashes_deep($layer);
				}
			}
			$this->layers = $layers;
			//$this->layers = AdamLabsGallery_Base::stripslashes_deep(@json_decode($skin['layers'], true));
			$settings = AdamLabsGallery_Base::stripslashes_deep(@json_decode($skin['settings'], true));
     ...
     */
    public static function get_adamlabsgallery_item_skins($type = 'all', $do_decode = true){
        global $wpdb;
		...
		if(!empty($item_skins) && $do_decode){
			foreach($item_skins as $key => $skin){
				$item_skins[$key]['params'] = AdamLabsGallery_Base::stripslashes_deep(@json_decode($item_skins[$key]['params'], true));
				...
				//$item_skins[$key]['layers'] = AdamLabsGallery_Base::stripslashes_deep(@json_decode($item_skins[$key]['layers'], true));
				$item_skins[$key]['settings'] = AdamLabsGallery_Base::stripslashes_deep(@json_decode($item_skins[$key]['settings'], true));
			}
		}
		
		return apply_filters('adamlabsgallery_get_adamlabsgallery_item_skins', $item_skins, $type, $do_decode);
    }
	...
	public static function get_handle_by_id($id = 0){
		...
		return apply_filters('adamlabsgallery_get_handle_by_id', $skin, $id);
	}
	...
	public static function get_id_by_handle($handle = ''){
		...
		return apply_filters('adamlabsgallery_get_id_by_handle', $skin, $handle);
	}
	 ...
	 */
	public static function get_adamlabsgallery_item_skin_by_id($id = 0){
		global $wpdb;
		...
		if(!empty($skin)){
			$skin['params'] = AdamLabsGallery_Base::stripslashes_deep(@json_decode($skin['params'], true));
			$layers = @json_decode($skin['layers'], true);
			if(!empty($layers) && is_array($layers)){ //prevent overhead
				foreach($layers as $lkey => $layer){
					$layers[$lkey] = AdamLabsGallery_Base::stripslashes_deep($layer);
			...
			$skin['layers'] = $layers;
			//$skin['layers'] = AdamLabsGallery_Base::stripslashes_deep(@json_decode($skin['layers'], true));
			$skin['settings'] = AdamLabsGallery_Base::stripslashes_deep(@json_decode($skin['settings'], true));
		}
		
		return apply_filters('adamlabsgallery_get_adamlabsgallery_item_skin_by_id', $skin, $id);
	}
	...
	public function switch_item_skin($skin_id){
		...
		$skin_id = apply_filters('adamlabsgallery_switch_item_skin', $skin_id, $this->loaded_skins);
		...
		if($skin_id == -1){
			...
			if(!isset($this->loaded_skins[$skin_id])){
				//2. If not, get it and set it with: get_adamlabsgallery_item_skin_by_id()
				$skin = $this->get_adamlabsgallery_item_skin_by_id($skin_id);
				if(empty($skin)){
					$to_default = true;
				}else{
	...
	public function add_to_skin_list($data){
		$data = apply_filters('adamlabsgallery_add_to_skin_list', $data);
    ...
    public static function update_save_item_skin($data){
		...
		$data = apply_filters('adamlabsgallery_update_save_item_skin', $data);
        ...
        if(isset($data['name'])){
            ...
            return __('Invalid name. Name has to be at least 2 characters long.', ADAMLABS_GALLERY_TEXTDOMAIN);
        }
        
        if(isset($data['id'])){
            if(intval($data['id']) == 0) return __('Invalid Item Template. Wrong ID given.', ADAMLABS_GALLERY_TEXTDOMAIN);
        }
        ...
        if(!isset($data['params']) || empty($data['params'])) return __('No parameters found.', ADAMLABS_GALLERY_TEXTDOMAIN);
        ...
        if(isset($data['id']) && intval($data['id']) > 0){ //update
            ...
            if(!empty($check)){
                return __('Item template with chosen name already exist. Please use a different name.', ADAMLABS_GALLERY_TEXTDOMAIN);
            }
			...
			if(!empty($skin)){
				...
				if($response === false) return __('Item template could not be changed.', ADAMLABS_GALLERY_TEXTDOMAIN);
	...
	public static function delete_item_skin_by_id($data){
		...
		$data = apply_filters('adamlabsgallery_delete_item_skin_by_id', $data);
		...
		if($response === false) return __('Item Template could not be deleted', ADAMLABS_GALLERY_TEXTDOMAIN);
    ...
    public function sort_item_skins(){
        ...
        if(!empty($this->layers)){
            ...
            if(count($this->layers) >= 2)
                usort($this->layers, array('AdamLabsGallery_Base', 'sort_by_order'));
        }
        
		$this->layers = apply_filters('adamlabsgallery_sort_item_skins', $this->layers);
    }
    ...
    public function set_lazy_load($set_to){
		...
		$this->lazy_load = apply_filters('adamlabsgallery_set_lazy_load', $set_to);
    ...
    public function set_lazy_load_blur($set_to){
		...
		$this->lazy_load_blur = apply_filters('adamlabsgallery_set_lazy_load_blur', $set_to);
    ...
    public function set_grid_type($grid_type){
		...
		$this->grid_type = apply_filters('adamlabsgallery_set_grid_type', $grid_type);
    ...
    public function set_lightbox_rel($rel){
		...
		$this->lb_rel = apply_filters('adamlabsgallery_set_lightbox_rel', $rel);
    ...
    public function set_default_lightbox_source_order($order){
		...
		$this->default_lightbox_source_order = apply_filters('adamlabsgallery_set_default_lightbox_source_order', $order);
    ...
    public function set_default_ajax_source_order($order){
		...
		$this->default_ajax_source_order = apply_filters('adamlabsgallery_set_default_ajax_source_order', $order);
    ...
    public function set_default_media_source_order($order){
		...
		$this->default_media_source_order = apply_filters('adamlabsgallery_set_default_media_source_order', $order);
    ...
    public function set_default_video_poster_order($order){
		...
		$this->default_video_poster_order = apply_filters('adamlabsgallery_set_default_video_poster_order', $order);
    ...
    public function set_poster_cropping($set_to){
		...
		$this->do_poster_cropping = apply_filters('adamlabsgallery_set_poster_cropping', $set_to);
	...
	public function set_lightbox_addition($addition){
		...
		$this->lightbox_additions = apply_filters('adamlabsgallery_set_lightbox_addition', $addition);
	...
	public function set_fancybox_three_options($title){
		...
		$this->fancybox_three_options = apply_filters('adamlabsgallery_set_fancybox_three_options', $options);
    ...
    public function set_video_ratios($video_ratios){
		...
		$video_ratios = apply_filters('adamlabsgallery_set_video_ratios', $video_ratios);
		...
		$this->video_ratios = apply_filters('adamlabsgallery_set_video_ratios_set', $this->video_ratios, $video_ratios);
    }
    ...
    public function set_sorting($data){
		...
		$this->sorting = apply_filters('adamlabsgallery_set_sorting', $this->sorting, $data);
    }
	...
	public static function star_item_skin_by_id($data){
		...
		$data = apply_filters('adamlabsgallery_star_item_skin_by_id', $data);
        ...
        if(empty($item_skin)) return __('Invalid Skin', ADAMLABS_GALLERY_TEXTDOMAIN);
		...
		if($response === false) return __('Could not change Favorite', ADAMLABS_GALLERY_TEXTDOMAIN);
	...
	public static function duplicate_item_skin_by_id($data){
		...
		if(empty($duplicate))
			return __('Item Template could not be duplicated', ADAMLABS_GALLERY_TEXTDOMAIN);
		...
		do {
			$i++;
			$result = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table_name WHERE name = %s", 'Item Template '.$i), ARRAY_A);
			
		} while(!empty($result));

		//now add new Entry
		unset($duplicate['id']);
		$duplicate['name'] = 'Item Template '.$i;
		$duplicate['handle'] = 'item-template-'.$i;
		
		$duplicate = apply_filters('adamlabsgallery_duplicate_item_skin_by_id', $duplicate);
		
		$response = $wpdb->insert($table_name, $duplicate);
	
		if($response === false) return __('Item Template could not be duplicated', ADAMLABS_GALLERY_TEXTDOMAIN);
	...
	public static function propagate_default_item_skins($networkwide = false){
		...
		if(function_exists('is_multisite') && is_multisite() && $networkwide){ //do for each existing site
            ...
            foreach($blogids as $blog_id){
				...
				$skins = apply_filters('adamlabsgallery_propagate_default_item_skins_multisite', $skins, $blog_id);
			...
			$skins = apply_filters('adamlabsgallery_propagate_default_item_skins', $skins);
	...
	public static function get_default_item_skins(){
		...
		$default = apply_filters('adamlabsgallery_add_default_item_skins', $default); //backwards compatibility
		$default = apply_filters('adamlabsgallery_get_default_item_skins', $default);
	...
	public static function insert_default_item_skins($data){
		...
		$data = apply_filters('adamlabsgallery_insert_default_item_skins', $data);
	...
	public function set_load_more(){
		...
		$this->load_more_element = apply_filters('adamlabsgallery_set_load_more', true);
	...
	public function get_skin_layer(){
		...
		return apply_filters('adamlabsgallery_get_skin_layer', $this->layers);
	...
	public function get_skin_id(){
		...
		return apply_filters('adamlabsgallery_get_skin_id', $this->id);
    ...
    public function output_item_skin($demo = false, $choosen_skin = 0){
		...
		$container_class = ' adamlabsgallery-'.esc_attr($this->handle).'-container';
		$li_class = ' adamlabsgallery-'.esc_attr($this->handle).'-wrapper';
		$li_skin = ' data-skin="' . esc_attr($this->handle) . '"';
		
		$li_class .= ($is_post) ? ' adamlabsgallery-post-id-'.@$this->post['ID'] : ' adamlabsgallery-post-id-'.$this->item_counter;
		$li_id = ($is_post) ? 'adamlabsgallery-'.$this->grid_id.'-post-id-'.@$this->post['ID'] : 'adamlabsgallery-'.$this->grid_id.'-post-id-'.$this->item_counter;
		...
		if($demo === false || $demo === 'preview') {
			...
			if($is_post) {
				...
				$meta216 = $m->get_meta_value_by_handle($this->post['ID'], 'adamlabsgallery_custom_meta_216');
				...
				if($meta216 != 'true') {
					...
					$color_processed = AdamLabsColorpicker::process($container_background_color);
					...
					if(!empty($color_processed) && is_array($color_processed) && count($color_processed) > 1) {
						...
						if($meta_cover_bg_color === false && $meta_cover_bg_opacity !== false) { 
							...
							if($color_type === 'rgb' || $color_type === 'rgba') {
								$rgb_values = AdamLabsColorpicker::rgbValues($container_background_color, 3);
								...
								$meta_cover_bg_color = AdamLabsColorpicker::convert($container_background_color, $meta_cover_bg_opacity);
							}
						}
						else if($meta_cover_bg_color !== false && $meta_cover_bg_opacity === false) {
							
							if($color_type === 'rgb' || $color_type === 'rgba') {
								$rgb_values = AdamLabsColorpicker::rgbValues($container_background_color, 4);
								if(!empty($rgb_values) && is_array($rgb_values) && count($rgb_values) > 3) {
									$meta_cover_bg_color = AdamLabsColorpicker::processRgba($meta_cover_bg_color, $rgb_values[3]);
								}
							}
						}
						else if($meta_cover_bg_color !== false && $meta_cover_bg_opacity !== false) { 		
							$meta_cover_bg_color = AdamLabsColorpicker::convert($meta_cover_bg_color, $meta_cover_bg_opacity);
						}	
					}
				}
			}
		...
		if($meta_cover_bg_color !== false) $meta_cover_bg_color = AdamLabsColorpicker::get($meta_cover_bg_color);
		if($meta_content_bg_color !== false) $meta_content_bg_color = AdamLabsColorpicker::get($meta_content_bg_color);
		if($meta_item_bg_color !== false) $meta_item_bg_color = AdamLabsColorpicker::get($meta_item_bg_color);
		...
		$cover_blend_mode = $cover_blend_mode === 'normal' ? '' : ' adamlabsgallery-cover-blend-' . $cover_blend_mode;
        ...
        if($cover_type == 'full'){ //cover is for overlay container
			...
			if(empty($force_show_cover) || $force_show_cover === 'false') {
				$cover_animation_center = 'adamlabsgallery-'.$base->getVar($this->params, 'cover-animation-center', 'fade').$base->getVar($this->params, 'cover-animation-center-type', '');
				
				/* 2.2.6 */
				if(preg_match('/spiral|circle/', $cover_animation_center)) $cover_wrapper_overflow = ' adamlabsgallery-cover-overflow';
				...
				$cover_animation_center = 'adamlabsgallery-none';
			}
            
            if($cover_animation_center != 'adamlabsgallery-none' && $cover_animation_center != ' adamlabsgallery-noneout') {
			...
			/* 2.1.6 */
			if(empty($force_show_cover) || $force_show_cover === 'false') {
				$cover_animation_top = 'adamlabsgallery-'.$base->getVar($this->params, 'cover-animation-top', 'fade').$base->getVar($this->params, 'cover-animation-top-type', '');
				$cover_animation_center = 'adamlabsgallery-'.$base->getVar($this->params, 'cover-animation-center', 'fade').$base->getVar($this->params, 'cover-animation-center-type', '');
				$cover_animation_bottom = 'adamlabsgallery-'.$base->getVar($this->params, 'cover-animation-bottom', 'fade').$base->getVar($this->params, 'cover-animation-bottom-type', '');
				...
				if(preg_match('/spiral|circle/', $cover_animation_top) || preg_match('/spiral|circle/', $cover_animation_center) || preg_match('/spiral|circle/', $cover_animation_bottom)) {
					$cover_wrapper_overflow = ' adamlabsgallery-cover-overflow';
				}
				...
				$cover_animation_top = 'adamlabsgallery-none';
				$cover_animation_center = 'adamlabsgallery-none';
				$cover_animation_bottom = 'adamlabsgallery-none';
			}
			
            if($cover_animation_top != 'adamlabsgallery-none' && $cover_animation_top != ' adamlabsgallery-noneout') {
            ...
            if($cover_animation_bottom != 'adamlabsgallery-none' && $cover_animation_bottom != ' adamlabsgallery-noneout') {
		...
		if($cover_animation_top) {
			...
			$cover_animation_top = ' adamlabsgallery-transition';
		...
		if($cover_animation_center) {
			...
			$cover_animation_center = ' adamlabsgallery-transition';
		...
		if($cover_animation_bottom) {
			...
			$cover_animation_bottom = ' adamlabsgallery-transition';
		...
		*/
		...
		if(empty($disable_group_animation)) {
			$cover_group_animation = 'adamlabsgallery-'.$base->getVar($this->params, 'cover-group-animation', 'fade');
		}
		else {
			$cover_group_animation = 'adamlabsgallery-none';
		}
		
        if($cover_group_animation != 'adamlabsgallery-none') {
		...
		if($cover_group_animation) {
			...
			$cover_group_animation = ' adamlabsgallery-transition';
        ...
        $media_animation = 'adamlabsgallery-'.$base->getVar($this->params, 'media-animation', 'fade');
		$media_blur = strpos($media_animation, 'blur') === false ? '' : ' data-bluramount="' . $base->getVar($this->params, 'media-animation-blur', '5') . '"';
		
        if($media_animation != 'adamlabsgallery-none') {
		...
		if($this->load_more_element == true) $li_class .= ' adamlabsgallery-newli';
		
		/* 2.1.6 Split Item Option */
		$splitItem = $base->getVar($this->params, 'splitted-item', 'none');
		if(!empty($splitItem) && $splitItem !== 'none') {
			$li_class .= ' adamlabsgallery-split-content adamlabsgallery-split-' . $splitItem;
		}
		...
		if($itm_anime !== 'none') {
			$item_animation .= ' data-anime="adamlabsgallery-item-' . $itm_anime . '"';
			switch($itm_anime) {
				case 'zoomin':
		...
		if($itm_anime_other !== 'none') {
			$item_animation .= ' data-anime-other="adamlabsgallery-item-' . $itm_anime_other . '"';
		...
		if($demo === 'skinchoose') {
			...
			$li_class .= ' adamlabsgallery-tooltip-wrap';
			$cltitle = 'title="'.__('Select Skin', ADAMLABS_GALLERY_TEXTDOMAIN). '" ';
			
		}
		
        echo '<li ' . $cltitle . 'id="'.$li_id.'"'.$li_skin.' class="filterall'.$filters.$li_class;
		if($demo == 'custom') echo ' adamlabsgallery-newli'; //neccesary for refresh of preview grid if new li will be added
        ...
        if($demo == 'overview' || $demo == 'skinchoose'){
			...
			$cl = ($demo == 'skinchoose') ? 'adamlabsgallery-screenselect-toolbar' : ''; //show only in grid editor at skin chooser
            ...
            echo '<div class="adamlabsgallery-item-skin-overview-name">'.$this->name.$showid."</div>\n";
			
			if($demo == 'overview'){
				$fav_class = (!isset($this->settings['favorite']) || $this->settings['favorite'] == false) ? 'adamlabsgallery-icon-star-empty' : 'adamlabsgallery-icon-star';
				
				echo '<a href="javascript:void(0);" title="'.__('Mark as Favorite', ADAMLABS_GALLERY_TEXTDOMAIN).'" class="adamlabsgallery-ov-1 adamlabsgallery-overview-button adamlabsgallery-btn-star-item-skin revyellow adamlabsgallery-tooltip-wrap" id="adamlabsgallery-star-'. $this->id .'"><i class="'.$fav_class.'"></i></a>';
				echo '<a href="'.AdamLabsGallery_Base::getViewUrl(AdamLabsGallery_Admin::VIEW_ITEM_SKIN_EDITOR, 'create='.$this->id).'" title="'.__('Edit Template', ADAMLABS_GALLERY_TEXTDOMAIN).'" class="adamlabsgallery-tooltip-wrap adamlabsgallery-ov-2 adamlabsgallery-overview-button revgreen "><i class="adamlabsgallery-icon-cog"></i></a>';
                echo '<a href="javascript:void(0);" title="'.__('Duplicate Template', ADAMLABS_GALLERY_TEXTDOMAIN). ($GLOBALS['adamlabsgallery_validated'] === 'false' ? ' (Pro)' : '') . '" class="adamlabsgallery-ov-3 adamlabsgallery-overview-button '. ($GLOBALS['adamlabsgallery_validated'] === 'true' ? 'adamlabsgallery-btn-duplicate-item-skin' : 'disabled') .' revcarrot adamlabsgallery-tooltip-wrap " id="adamlabsgallery-duplicate-'. $this->id .'"><i class="adamlabsgallery-icon-picture"></i></a>';

				echo '<a href="javascript:void(0);" title="'.__('Delete Template', ADAMLABS_GALLERY_TEXTDOMAIN).'" class="adamlabsgallery-ov-4 adamlabsgallery-overview-button adamlabsgallery-btn-delete-item-skin revred adamlabsgallery-tooltip-wrap " id="adamlabsgallery-delete-'. $this->id .'"><i class="adamlabsgallery-icon-trash"></i></a>';
			}elseif($demo == 'skinchoose'){
				echo '<a href="'.($GLOBALS['adamlabsgallery_validated'] === 'true' ? 'admin.php?page=adamlabsgallery&view=grid-item-skin-editor&create='. $this->id : '#') . '" '.($GLOBALS['adamlabsgallery_validated'] !== 'true' ? 'onclick="alert(\'This is a PRO feature, please activate PRO license to use it \'); return false;"': '' ).' class="adamlabsgallery-edit-skin-button adamlabsgallery-overview-button revyellow adamlabsgallery-tooltip-wrap" '.($GLOBALS['adamlabsgallery_validated'] === 'true' ? 'target="_blank"' : '').' title="' . __('Edit Template', ADAMLABS_GALLERY_TEXTDOMAIN) . ($GLOBALS['adamlabsgallery_validated'] !== 'true' ? '(PRO)' : '') . ' "><i class="adamlabsgallery-icon-tint"></i></a>';
				echo '<input class="adamlabsgallery-tooltip-wrap " style="position: absolute; right: 0; top: 0;" type="radio" value="'.$this->id.'" title="'. __('Choose Template', ADAMLABS_GALLERY_TEXTDOMAIN).'" name="entry-skin"';
				if($choosen_skin == '-1')
			...
			$vis_icon = ($is_visible) ? 'adamlabsgallery-icon-eye' : 'adamlabsgallery-icon-eye-off';
			$vis_icon_color = ($is_visible) ? 'revblue' : 'revred';			
			
			echo '<div class="adamlabsgallery-atoolbar" style="display:block;width:100%;height:30px;bottom:0px;left:0px;position:absolute;z-index:10; padding: 0;">'."\n";
            echo '          <div class="btn-wrap-item-skin-overview-'.$this->post['ID'].'">'."\n";
            echo '<div class="adamlabsgallery-item-skin-overview-name">';
			echo '<a href="javascript:void(0);" class="adamlabsgallery-ov-2 adamlabsgallery-overview-button adamlabsgallery-btn-activate-post-item '.$vis_icon_color.' adamlabsgallery-tooltip-wrap" title="'.__('Show/Hide from Grid', ADAMLABS_GALLERY_TEXTDOMAIN).'" id="adamlabsgallery-act-post-item-'. $this->post['ID'] .'"><i class="'.$vis_icon.'"></i></a>';
			echo '<a href="'.get_edit_post_link($this->post['ID']).'" class="adamlabsgallery-ov-3 adamlabsgallery-overview-button revyellow adamlabsgallery-tooltip-wrap" title="'.__('Edit Post', ADAMLABS_GALLERY_TEXTDOMAIN).'" target="_blank"><i class="adamlabsgallery-icon-pencil-1"></i></a>';
			echo '<a href="javascript:void(0);" class="adamlabsgallery-ov-4 adamlabsgallery-overview-button adamlabsgallery-btn-edit-post-item revgreen adamlabsgallery-tooltip-wrap" title="'.__('Edit Post Meta', ADAMLABS_GALLERY_TEXTDOMAIN).'" id="adamlabsgallery-edit-post-item-'. $this->post['ID'] .'"><i class="adamlabsgallery-icon-cog"></i></a>';
			echo '</div>'."\n";
			echo '          </div>'."\n";
            echo '          <div class="clear"></div>'."\n\n";
            echo '       </div>'."\n\n";
			...
			echo '<div class="adamlabsgallery-data-handler" data-exists="'.$custom_layer_elements.'" style="display: none;"></div>'; //has the information on what exists as layers in the skin #3498DB
			
			echo '<div class="adamlabsgallery-atoolbar" style="display:block;width:100%;height:30px;bottom:0px;left:0px;position:absolute;z-index:10;background-color: transparent; padding: 0;">'."\n";
			echo '          <div class="btn-wrap-item-skin-overview-0">'."\n";
			echo '<div class="adamlabsgallery-item-skin-overview-name">';
			
			echo '<a href="javascript:void(0);" title="'.__('Move', ADAMLABS_GALLERY_TEXTDOMAIN).'" style="cursor: move;" class="adamlabsgallery-ov-10 adamlabsgallery-overview-button adamlabsgallery-tooltip-wrap "><i class="adamlabsgallery-icon-menu"></i></a>';
			echo '<a href="javascript:void(0);" title="'.__('Move one before', ADAMLABS_GALLERY_TEXTDOMAIN).'" class="adamlabsgallery-ov-11 adamlabsgallery-overview-button adamlabsgallery-btn-move-before-custom-element adamlabsgallery-tooltip-wrap "><i class="adamlabsgallery-icon-angle-left"></i></a>';
			echo '<a href="javascript:void(0);" title="'.__('Move one after', ADAMLABS_GALLERY_TEXTDOMAIN).'" class="adamlabsgallery-ov-12 adamlabsgallery-overview-button adamlabsgallery-btn-move-after-custom-element adamlabsgallery-tooltip-wrap "><i class="adamlabsgallery-icon-angle-right"></i></a>';
			echo '<a href="javascript:void(0);" title="'.__('Move after #x', ADAMLABS_GALLERY_TEXTDOMAIN).'" class="adamlabsgallery-ov-13 adamlabsgallery-overview-button adamlabsgallery-btn-switch-custom-element adamlabsgallery-tooltip-wrap "><i class="adamlabsgallery-icon-angle-double-right"></i></a>';
			
			echo '<a href="javascript:void(0);" title="'.__('Delete Element', ADAMLABS_GALLERY_TEXTDOMAIN).'" class="adamlabsgallery-ov-4 adamlabsgallery-overview-button adamlabsgallery-btn-delete-custom-element adamlabsgallery-tooltip-wrap "><i class="adamlabsgallery-icon-trash"></i></a>';
			echo '<a href="javascript:void(0);" title="'.__('Duplicate Element', ADAMLABS_GALLERY_TEXTDOMAIN).'" class="adamlabsgallery-ov-3 adamlabsgallery-overview-button adamlabsgallery-btn-duplicate-custom-element adamlabsgallery-tooltip-wrap "><i class="adamlabsgallery-icon-picture"></i></a>';
			echo '<a href="javascript:void(0);" title="'.__('Edit Element', ADAMLABS_GALLERY_TEXTDOMAIN).'" class="adamlabsgallery-ov-2 adamlabsgallery-overview-button adamlabsgallery-btn-edit-custom-element adamlabsgallery-tooltip-wrap "><i class="adamlabsgallery-icon-cog"></i></a>';
		...
		if($demo == false || $demo == 'preview' || $demo == 'custom'){
					...
					if(isset($this->media_sources[$order]) && $this->media_sources[$order] !== '' && $this->media_sources[$order] !== false){ //found entry
						...
						switch($order){
								...
								if($this->lazy_load){
									...
									if($this->lazy_load_blur){
										if(isset($this->media_sources['alternate-image-preload-url']) && !empty($this->media_sources['alternate-image-preload-url'])){
											...
											$small_thumb = adamlabsgallery_aq_resize($this->media_sources[$order], 25, 25, true, true, true);
									...
									$echo_media = '<img src="'.ADAMLABS_GALLERY_PLUGIN_URL.'com/public/assets/images/300x200transparent.png"'.$small_thumb.' data-lazysrc="'.esc_attr($this->media_sources[$order]).'" alt="'.esc_attr($this->media_sources[$order.'-alt']).'"'.$img_dim.'>';
								}else{
							...
							case 'content-youtube':
								...
								$video_poster_src = ($this->do_poster_cropping == true) ? adamlabsgallery_aq_resize($video_poster_src, $this->video_sizes[$this->video_ratios['youtube']]['width'], $this->video_sizes[$this->video_ratios['youtube']]['height'], true, true, true) : $video_poster_src;
								$lightbox_thumb = $video_poster_src;
								
								$echo_media = '<div class="adamlabsgallery-media-video" data-youtube="'.esc_attr($this->media_sources[$order]).'" width="'.esc_attr($this->video_sizes[$this->video_ratios['youtube']]['width']).'" height="'.esc_attr($this->video_sizes[$this->video_ratios['youtube']]['height']).'" data-poster="'.esc_attr($video_poster_src).'"></div>';
								$is_video = true;
							break;
							case 'vimeo':
							case 'content-vimeo':
								...
								//if we are masonry, we need to crop the image
								$video_poster_src = ($this->do_poster_cropping == true) ? adamlabsgallery_aq_resize($video_poster_src, $this->video_sizes[$this->video_ratios['vimeo']]['width'], $this->video_sizes[$this->video_ratios['vimeo']]['height'], true, true, true) : $video_poster_src;
								$lightbox_thumb = $video_poster_src;
								
								$echo_media = '<div class="adamlabsgallery-media-video" data-vimeo="'.esc_attr($this->media_sources[$order]).'" width="'.esc_attr($this->video_sizes[$this->video_ratios['vimeo']]['width']).'" height="'.esc_attr($this->video_sizes[$this->video_ratios['vimeo']]['height']).'" data-poster="'.esc_attr($video_poster_src).'"></div>';
								$is_video = true;
							...
							case 'content-wistia':
								//if we are masonry, we need to crop the image
								$video_poster_src = ($this->do_poster_cropping == true) ? adamlabsgallery_aq_resize($video_poster_src, $this->video_sizes[$this->video_ratios['wistia']]['width'], $this->video_sizes[$this->video_ratios['wistia']]['height'], true, true, true) : $video_poster_src;
								$lightbox_thumb = $video_poster_src;
								
								$echo_media = '<div class="adamlabsgallery-media-video" data-wistia="'.esc_attr($this->media_sources[$order]).'" width="'.esc_attr($this->video_sizes[$this->video_ratios['wistia']]['width']).'" height="'.esc_attr($this->video_sizes[$this->video_ratios['wistia']]['height']).'" data-poster="'.esc_attr($video_poster_src).'"></div>';
							...
							case 'content-html5':
								...
								//if we are masonry, we need to crop the image
								$video_poster_src = ($this->do_poster_cropping == true) ? adamlabsgallery_aq_resize($video_poster_src, $this->video_sizes[$this->video_ratios['html5']]['width'], $this->video_sizes[$this->video_ratios['html5']]['height'], true, true, true) : $video_poster_src;
								$lightbox_thumb = $video_poster_src;
								
								$echo_media = '<div class="adamlabsgallery-media-video" data-mp4="'.esc_attr(@$this->media_sources[$order]['mp4']).'" data-webm="'.esc_attr(@$this->media_sources[$order]['webm']).'" data-ogv="'.esc_attr(@$this->media_sources[$order]['ogv']).'" width="'.esc_attr($this->video_sizes[$this->video_ratios['html5']]['width']).'" height="'.esc_attr($this->video_sizes[$this->video_ratios['html5']]['height']).'" data-poster="'.esc_attr($video_poster_src).'"></div>';
								$is_video = true;
							break;
							case 'soundcloud':
							case 'content-soundcloud':
								//if we are masonry, we need to crop the image
								$video_poster_src = ($this->do_poster_cropping == true) ? adamlabsgallery_aq_resize($video_poster_src, $this->video_sizes[$this->video_ratios['soundcloud']]['width'], $this->video_sizes[$this->video_ratios['soundcloud']]['height'], true, true, true) : $video_poster_src;
								$lightbox_thumb = $video_poster_src;
								
								$echo_media = '<div class="adamlabsgallery-media-video" data-soundcloud="'.esc_attr($this->media_sources[$order]).'" width="'.esc_attr($this->video_sizes[$this->video_ratios['soundcloud']]['width']).'" height="'.esc_attr($this->video_sizes[$this->video_ratios['soundcloud']]['height']).'" data-poster="'.esc_attr($video_poster_src).'"></div>';
								$is_video = true;
							break;
							case 'iframe':
						...
						$echo_media = apply_filters('adamlabsgallery_set_media_source', $echo_media, $order, @$this->media_sources);
						$is_iframe = apply_filters('adamlabsgallery_set_media_source_is_iframe', $is_iframe, $order);
						$is_video = apply_filters('adamlabsgallery_set_media_source_is_video', $is_video, $order);
						$video_poster_src = apply_filters('adamlabsgallery_set_media_source_video_poster_src', $video_poster_src, $order, @$this->video_sizes, @$this->video_ratios);
						$do_continue = apply_filters('adamlabsgallery_set_media_source_do_continue', $do_continue, $order);
			...
			if($echo_media == ''){ //set default image if one is set
				...
				// if(empty($echo_media)) $echo_media = '<img class="adamlabsgallery-no-media" src="'. ADAMLABS_GALLERY_PLUGIN_URL .'com/public/assets/images/300x200transparent.png">';
			...
			if(!empty($hover_image) && $hover_image !== 'false' && !empty($this->media_sources) && isset($this->media_sources['alternate-image']) && !empty($this->media_sources['alternate-image'])) {	
				...
				$hover_image_animation = 'adamlabsgallery-'.$base->getVar($this->params, 'hover-image-animation', 'fade');
				
				if($hover_image_animation != 'adamlabsgallery-none') {
				...
				if($hover_image_animation) {
					...
					$hover_image_animation = ' adamlabsgallery-transition';
				...
				$echo_media .= '<div class="adamlabsgallery-hover-image' . $hover_image_animation . '" data-src="' . $hover_image . '"' . $hover_image_animation_delay . $data_transition_hover . '></div>';
			}
		...
		if($link_set_to !== 'none'){
			...
			switch($link_type_link){
				...
				case 'lightbox':
					...
					if(!AdamLabsGallery_Jackbox::is_active() && !AdamLabsGallery_Social_Gallery::is_active() && $opt !== 'disabled'){ //enqueue only if default LightBox is selected
					...
					$lb_featured = '';
					$lb_post_title = '';
					$lb_rel = ($this->lb_rel !== false) ? ' data-adamlabsgallerybox="'.esc_attr($this->lb_rel).'"' : '';
									...
									switch($order){
										...
										case 'html5':
											if(trim($this->media_sources[$order]['mp4']) === '' && trim($this->media_sources[$order]['ogv']) === '' && trim($this->media_sources[$order]['webm'] === '')){
												...
												$lb_class = ' adamlabsgallerybox adamlabsgalleryboxhtml5';
										...
										case 'post-content':
											...
											$lb_source = 'javascript:void(0);';
											$lb_class = ' adamlabsgallerybox adamlabsgallerybox-post';
										...
										case 'adamlabsgallery':
											...
											$lb_source = admin_url('admin-ajax.php');
											$lb_class = ' adamlabsgallerybox adamlabsgallerybox-post';
											$lb_data = ' data-post="' . $post_ids . '" data-lbadamlabsgallery="' . $this->media_sources[$order] . '"';
											$lb_data .= ' data-gridid="' . $grid_ids . '" data-ispost="' . $is_post . '"';
					...
					if($demo !== false){
						$lb_title = __('demo mode', ADAMLABS_GALLERY_TEXTDOMAIN);
					}else{
						if($is_post)
				...
				case 'ajax':
					...
					if(!empty($this->default_ajax_source_order)){ //only show if something is checked
						$ajax_class = ' adamlabsgallery-ajaxclicklistener';
						foreach($this->default_ajax_source_order as $order){ //go through the order and set media as wished
							...
							if(isset($this->media_sources[$order]) && $this->media_sources[$order] !== '' && $this->media_sources[$order] !== false || $order == 'post-content'){ //found entry
								...
								$ajax_attr = apply_filters('adamlabsgallery_handle_ajax_content', $order, $this->media_sources, $postobj, $this->grid_id);
								if(empty($ajax_attr)){
									//$ajax_class = '';
									$do_continue = true;
								}
		...
		if($media_animation) {
			...
			$media_animation = ' adamlabsgallery-transition';
        ...
        echo '    <div class="adamlabsgallery-media-cover-wrapper' . $cover_wrapper_overflow . '">'."\n";
        //echo '            <!-- THE MEDIA OF THE ENTRY -->'."\n";
		if($demo == 'overview' || $demo == 'skinchoose'){
			echo '            <div class="adamlabsgallery-entry-media'.$media_animation.'"'.$media_animation_delay.$media_animation_duration.$media_blur.$data_transition_media.'><img src="'.ADAMLABS_GALLERY_PLUGIN_URL.'com/admin/assets/images/'.$this->cover_image.'"></div>'."\n\n";
		}else{
			$echo_media = '<div class="adamlabsgallery-entry-media'.$media_animation.'"'.$media_animation_delay.$media_animation_duration.$media_blur.$data_transition_media.'>'.$echo_media.'</div>'."\n\n";
		...
		if($is_iframe == false){ //if we are iFrame, no wrapper and no elements in media should be written
			...
			if($cover_type == 'full' && $c_layer > 0 || ($t_layer > 0 || $c_layer > 0 || $b_layer > 0)){
				...
				echo '            <div class="adamlabsgallery-entry-cover'.$cover_group_animation.'"'.$cover_group_animation_delay.$cover_group_animation_duration.$cover_attr.$data_transition_group.'>'."\n\n";
				//echo '                <!-- THE COLORED OVERLAY -->'."\n";
				
				if($link_set_to == 'cover' && $link_type_link !== 'none'){
					if(strpos($link_wrapper, 'class="') !== false){
						echo str_replace(array('%REPLACE%', 'class="'), array('', 'class="adamlabsgallery-invisiblebutton '), $link_wrapper);
					}else{
						echo str_replace(array('%REPLACE%', '<a '), array('', '<a class="adamlabsgallery-invisiblebutton" '), $link_wrapper);
					}
					$link_inserted = true;
				}
			}
			if($cover_type == 'full'){
				$echo_c = '                <div class="adamlabsgallery-overlay'.$cover_animation_center.$container_class.$cover_blend_mode.'"'.$cover_animation_delay_center.$cover_animation_duration_center.$meta_cover_style.$data_transition_center.$cover_animation_color_center.'></div>'."\n\n";
				...
				if($t_layer > 0){
					$echo_t = '                <div class="adamlabsgallery-overlay adamlabsgallery-top'.$cover_animation_top.$container_class.$cover_blend_mode.'"'.$cover_animation_delay_top.$cover_animation_duration_top.$meta_cover_style.$data_transition_top.$cover_animation_color_top.'></div>'."\n\n";
					if($link_set_to == 'cover' && $link_type_link !== 'none' && $link_inserted === false){ //set link on whole cover
				...
				if($c_layer > 0){
					$echo_c = '                <div class="adamlabsgallery-overlay adamlabsgallery-center'.$cover_animation_center.$container_class.$cover_blend_mode.'"'.$cover_animation_delay_center.$cover_animation_duration_center.$meta_cover_style.$data_transition_center.$cover_animation_color_center.'></div>'."\n\n";
					if($link_set_to == 'cover' && $link_type_link !== 'none' && $link_inserted === false){ //set link on whole cover
						$echo_c = str_replace('%REPLACE%', $echo_c, $link_wrapper);
					}
					echo $echo_c;
				...
				if($b_layer > 0){
					$echo_b = '                <div class="adamlabsgallery-overlay adamlabsgallery-bottom'.$cover_animation_bottom.$container_class.$cover_blend_mode.'"'.$cover_animation_delay_bottom.$cover_animation_duration_bottom.$meta_cover_style.$data_transition_bottom.$cover_animation_color_bottom.'></div>'."\n\n";
					if($link_set_to == 'cover' && $link_type_link !== 'none' && $link_inserted === false){ //set link on whole cover
			...
			<!-- #########################################################################
					THE CLASSES FOR THE ALIGNS OF ANY ELEMENT IS:
					 ...
					 adamlabsgallery-top, adamlabsgallery-topleft, adamlabsgallery-topright,
					 adamlabsgallery-left, adamlabsgallery-right,  adamlabsgallery-center
					 adamlabsgallery-bottom, adamlabsgallery-bottomleft, adamlabsgallery-bottomright
			...
			*/
					...
					if($demo === false && $this->layer_values === false){ //show element only if it is on sale or if featured
						if(AdamLabsGallery_Woocommerce::is_woo_exists()){
							...
							if($show_on_sale == 'true'){
								$sale = AdamLabsGallery_Woocommerce::check_if_on_sale($this->post['ID']);
							...
							if($show_if_featured == 'true'){
								$featured = AdamLabsGallery_Woocommerce::check_if_is_featured($this->post['ID']);
			...
			if($this->load_lightbox === true){
				if(!empty($this->lightbox_additions['items'])){
					...
					echo '<div style="display: none" class="adamlabsgallerybox-additional">';
					foreach($this->lightbox_additions['items'] as $lb_key => $lb_img){
						...
						echo '<a href="'.esc_attr($lb_img).'" class="adamlabsgallerybox" '.$lb_rel.'><img class="adamlabsgallery-lb-dummy" src="'. ADAMLABS_GALLERY_PLUGIN_URL .'com/public/assets/images/300x200transparent.png"></a>';
					}
					echo '</div>';
				}
			}
	...
	public function output_add_more(){
			echo apply_filters('adamlabsgallery_output_add_more', '<li class="filterall adamlabsgallery-addnewitem-wrapper ui-state-disabled">
			<div class="adamlabsgallery-media-cover-wrapper">
				<div class="adamlabsgallery-entry-media"><img src="'. ADAMLABS_GALLERY_PLUGIN_URL .'com/public/assets/images/300x200transparent.png"></div>
				<div class="adamlabsgallery-entry-cover">
					<div class="adamlabsgallery-overlay adamlabsgallery-transition adamlabsgallery-addnewitem-container" data-transition="adamlabsgallery-fade" data-delay="0.18"></div>
					<div id="adamlabsgallery-add-new-custom-youtube" class="adamlabsgallery-open-edit-dialog adamlabsgallery-center adamlabsgallery-addnewitem-element-1 adamlabsgallery-transition" data-transition="adamlabsgallery-slideup" data-delay="0"><i class="adamlabsgallery-icon-youtube-squared"></i></div>
					<div class="adamlabsgallery-absolute adamlabsgallery-addnewitem-element-3 adamlabsgallery-transition" data-transition="adamlabsgallery-falldownout" data-delay="0.1"><i class="adamlabsgallery-icon-plus"></i></div>
					<div class="adamlabsgallery-bottom adamlabsgallery-addnewitem-element-2 adamlabsgallery-transition" data-transition="adamlabsgallery-flipup" data-delay="0.1">'. __('CHOOSE YOUR ITEM', ADAMLABS_GALLERY_TEXTDOMAIN) .'</div>
					<div id="adamlabsgallery-add-new-custom-vimeo" class="adamlabsgallery-open-edit-dialog adamlabsgallery-center adamlabsgallery-addnewitem-element-1 adamlabsgallery-addnewitem-element-space adamlabsgallery-transition" data-transition="adamlabsgallery-slideup" data-delay="0.1"><i class="adamlabsgallery-icon-vimeo-squared"></i></div>
					<div id="adamlabsgallery-add-new-custom-html5" class="adamlabsgallery-open-edit-dialog adamlabsgallery-center adamlabsgallery-addnewitem-element-1 adamlabsgallery-transition" data-transition="adamlabsgallery-slideup" data-delay="0.2" style="visibility: hidden"><i class="adamlabsgallery-icon-video"></i></div>
					<div class="adamlabsgallery-center adamlabsgallery-addnewitem-element-4 adamlabsgallery-none adamlabsgallery-clear" style="height: 5px; visibility: hidden;"></div>
					<div id="adamlabsgallery-add-new-custom-image" class="adamlabsgallery-open-edit-dialog adamlabsgallery-center adamlabsgallery-addnewitem-element-1 adamlabsgallery-transition" data-transition="adamlabsgallery-slideup" data-delay="0.3"><i class="adamlabsgallery-icon-picture-1"></i></div>
					<div id="adamlabsgallery-add-new-custom-soundcloud" class="adamlabsgallery-open-edit-dialog adamlabsgallery-center adamlabsgallery-addnewitem-element-1 adamlabsgallery-addnewitem-element-5 adamlabsgallery-transition" data-transition="adamlabsgallery-slideup" data-delay="0.4"><i class="adamlabsgallery-icon-soundcloud"></i></div>
					<div id="adamlabsgallery-add-new-custom-text" class="adamlabsgallery-open-edit-dialog adamlabsgallery-center adamlabsgallery-addnewitem-element-1 adamlabsgallery-addnewitem-element-6 adamlabsgallery-transition" data-transition="adamlabsgallery-slideup" data-delay="0.5"><i class="adamlabsgallery-icon-font"></i></div>
					<div id="adamlabsgallery-add-new-custom-blank" class="adamlabsgallery-open-edit-dialog adamlabsgallery-center adamlabsgallery-addnewitem-element-1 adamlabsgallery-transition" data-transition="adamlabsgallery-slideup" data-delay="0.6"><i class="adamlabsgallery-icon-cancel"></i></div>
				</div>
			</div>
		</li>');
	}
    ...
    public function insert_masonry_layer($demo = false, $style = false, $is_video = false, $grid_ids = '', $post_ids = ''){
		...
		$content_class = ' adamlabsgallery-'.esc_attr($this->handle).'-content';
		
        //$content_background_color = $base->getVar($this->params, 'content-bg-color', '#FFF');
        //echo '<!-- THE CONTENT PART OF THE ENTRIES -->'."\n";
		do_action('adamlabsgallery_insert_masonry_layer_pre', $demo, $style, $is_video);
		
        echo '<div class="adamlabsgallery-entry-content'.$content_class.'"';
		if($style !== false){
			echo apply_filters('adamlabsgallery_insert_masonry_layer_style', $style);
		}
        ...
        echo '</div>';
		do_action('adamlabsgallery_insert_masonry_layer_post', $demo, $style, $is_video);
    ...
    public function get_filter_array(){
		...
		return apply_filters('adamlabsgallery_get_filter_array', $this->filter);
    ...
    public function set_post_values($post){
		...
		$this->post = apply_filters('adamlabsgallery_set_post_values', $post);
    ...
    public function set_layer_values($values){
        ...
        $this->layer_values = apply_filters('adamlabsgallery_set_layer_values', $values);
    ...
    public function get_layer_values(){
        ...
        return apply_filters('adamlabsgallery_get_layer_values', $this->layer_values);
    ...
    public function set_post_meta_values(){
		...
		/* 2.2.6 */
			...
			$values = $this->post;
			$adamlabsgallery_settings_custom_meta_skin = isset($values['adamlabsgallery_settings_custom_meta_skin']) ? $values['adamlabsgallery_settings_custom_meta_skin'] : "";
			$adamlabsgallery_settings_custom_meta_element = isset($values['adamlabsgallery_settings_custom_meta_element']) ? $values['adamlabsgallery_settings_custom_meta_element'] : "";
			$adamlabsgallery_settings_custom_meta_setting = isset($values['adamlabsgallery_settings_custom_meta_setting']) ? $values['adamlabsgallery_settings_custom_meta_setting'] : "";
			$adamlabsgallery_settings_custom_meta_style = isset($values['adamlabsgallery_settings_custom_meta_style']) ? $values['adamlabsgallery_settings_custom_meta_style'] : "";
		...
		unset($values['adamlabsgallery_settings_custom_meta_skin']);
		unset($values['adamlabsgallery_settings_custom_meta_element']);
		unset($values['adamlabsgallery_settings_custom_meta_setting']);
		unset($values['adamlabsgallery_settings_custom_meta_style']);
		
		$values['adamlabsgallery-meta-style'] = $adamlabsgallery_meta;
		$this->post_meta = apply_filters('adamlabsgallery_set_post_meta_values', $values);
	...
	public function set_meta_element_changes($layer_id, $class){
		...
		if(!empty($this->post_meta) && !empty($this->post_meta['adamlabsgallery-meta-style'])){
			
			//get all allowed meta keys
			$item_ele = new AdamLabsGallery_Item_Element();
			$metas = $item_ele->get_allowed_meta();
			
			foreach($this->post_meta['adamlabsgallery-meta-style'] as $entry){
    ...
    public function generate_element_css($demo = false){
		...
		$allowed_wrap_styles = AdamLabsGallery_Item_Element::get_allowed_styles_for_wrap();
		$wait_for_styles = AdamLabsGallery_Item_Element::get_wait_until_output_styles();
		...
		if(!empty($this->layers_css)){
			foreach($this->layers_css as $layers_css){
				...
				if(!empty($this->add_css_tags)){
					$allowed_styles = AdamLabsGallery_Item_Element::get_allowed_styles_for_tags();
				...
				if(!empty($this->add_css_wrap)){
					$allowed_cat_tag_styles = AdamLabsGallery_Item_Element::get_allowed_styles_for_cat_tag();
		...
		if(!empty($this->media_css)){
			foreach($this->media_css as $skin_id => $media_css){
				if(!empty($media_css)){
					...
					$css .= '.adamlabsgallery-'.esc_attr($handle).'-wrapper .adamlabsgallery-entry-media-wrapper {'."\n";
					...
					if(isset($this->media_shadow[$skin_id])) {
						...
						$css .= '}'."\n";
						$css .= '.adamlabsgallery-'.esc_attr($handle).'-wrapper.adamlabsgallery-hovered .adamlabsgallery-entry-media-wrapper {'."\n";
		...
		if(!empty($this->cover_css)){
			foreach($this->cover_css as $skin_id => $cover_css){
				if(!empty($cover_css)){
					...
					$css = '';
					$handle = $this->loaded_skins[$skin_id]['handle'];
					$css .= '.adamlabsgallery-'.esc_attr($handle).'-container {'."\n";
					...
					if(isset($this->cover_shadow[$skin_id])) {
						...
						$css .= '	transition: box-shadow ' . $cover_duration . 's ease-out ' . $cover_delay . ';';
						$css .= '}'."\n";
						$css .= '.adamlabsgallery-hovered .adamlabsgallery-'.esc_attr($handle).'-container {'."\n";
		...
		if(!empty($this->content_css)){
			foreach($this->content_css as $skin_id => $content_css){
				if(!empty($content_css)){
					...
					$css = '';
					$handle = $this->loaded_skins[$skin_id]['handle'];
					$css .= '.adamlabsgallery-'.esc_attr($handle).'-content {'."\n";
					...
					if(isset($this->content_shadow[$skin_id])) {
						...
						$css .= '	transition: box-shadow ' . $cover_duration . 's ease-out ' . $cover_delay . ';';
						$css .= '}'."\n";
						$css .= '.adamlabsgallery-hovered .adamlabsgallery-'.esc_attr($handle).'-content{'."\n";
		...
		if(!empty($this->wrapper_css)){
			foreach($this->wrapper_css as $skin_id => $wrapper_css){
				if(!empty($wrapper_css)){
					...
					$css = '';
					$handle = $this->loaded_skins[$skin_id]['handle'];
					$css .= '.adamlabsgallery-grid .mainul li.adamlabsgallery-'.esc_attr($handle).'-wrapper {'."\n";
					...
					if(isset($this->wrapper_shadow[$skin_id])) {
						...
						$css .= '.adamlabsgallery-'.esc_attr($handle).'-wrapper {'."\n";
						$css .= '	transition: box-shadow ' . $cover_duration . 's ease-out ' . $cover_delay . ';';
						$css .= '}'."\n";
						$css .= '.adamlabsgallery-'.esc_attr($handle).'-wrapper.adamlabsgallery-hovered {'."\n";
		...
		if(!empty($this->media_poster_css)){
			foreach($this->media_poster_css as $skin_id => $media_poster_css){
				if(!empty($media_poster_css)){
					...
					$css = '';
					$handle = $this->loaded_skins[$skin_id]['handle'];
					$css .= '.adamlabsgallery-grid .mainul li.adamlabsgallery-'.esc_attr($handle).'-wrapper .adamlabsgallery-media-poster {'."\n";
	...
	public function output_element_css_by_meta($id = false, $grid_preview = false){
		...
		$p_class = ($id === false) ? '' : '.adamlabsgallery-post-' . $id;
		...
		if(!empty($this->layers_meta_css['idle'])){
            ...
            foreach($this->layers_meta_css['idle'] as $class => $settings){
                ...
                if(!empty($settings)){
					...
					// 2.2.6
					if(!empty($grid_preview) && !empty($this->grid_id)) $css .= '[id^="adamlabsgallery-grid-' . $this->grid_id . '"] ';
    ...
    public function register_skin_css(){
		...
		if(class_exists('AdamLabsColorpicker')) {
			...
			$container_background_color = AdamLabsColorpicker::get($container_background_color);
			$contentBgColor = AdamLabsColorpicker::get($contentBgColor);
			$fullBgColor = AdamLabsColorpicker::get($fullBgColor);
			
		}
		
        // $container_background_color_transparency = $base->getVar($this->params, 'element-container-background-color-opacity', '1');
        // $this->cover_css[$this->id]['background-color'] = AdamLabsGallery_Base::hex2rgba($container_background_color, $container_background_color_transparency); // we only need rgba in backend
		...
		// $shadow_rgba = AdamLabsGallery_Base::hex2rgba($shadow_color, $shadow_alpha);
	...
	public function register_layer_css($layer = false, $demo = false){
		...
		if($layer === false){
			...
			if(isset($layer['id'])) $unique_class = 'adamlabsgallery-'.esc_attr($this->handle).'-element-'.$layer['id'];
			else $unique_class = "";
				...
				switch($layer['settings']['source']){
					case 'post':
						...
						if($demo === false){
							...
							$post_text = AdamLabsGallery_Item_Element::getPostElementsArray();
						...
						$text = apply_filters('adamlabsgallery_post_meta_content', $text, $layer['settings']['source-post'], @$this->post['ID'], $this->post);
    ...
    public function add_element_css($settings, $element_class){
        ...
        if(!empty($settings)){
            $attributes = AdamLabsGallery_Item_Element::get_existing_elements(true);
            ...
            foreach($attributes as $style => $attr){
                ...
                if($attr['type'] == 'multi-text'){
                    ...
                    if($set_style == 'box-shadow' || $set_style == 'background-color'){
                        ...
                        $shadow_color = AdamLabsGallery_Base::hex2rgba($shadow_color, $shadow_transparency);
                        */
                    ...
                    if($set_style == 'background-color' || $set_style == 'background'){
                        ...
                        $bg_color_rgba = AdamLabsGallery_Base::hex2rgba($settings[$style], $bg_color_transparency); // we only need rgba in backend
						*/
						$bg_color_rgba = $settings[$style];
						if(class_exists('AdamLabsColorpicker')) $bg_color_rgba = AdamLabsColorpicker::get($bg_color_rgba);
    ...
    public function set_default_image_by_id($img_id){
		...
		/* 2.1.5 */
		if($img === false) {
			$img = get_option('adamlabsgallery_global_default_img', '');
    ...
    public function register_google_fonts(){
		...
		if(!empty($this->google_fonts)){
			foreach($this->google_fonts as $font){
				if($font !== ''){
					wp_register_style('adamlabsgallery-google-font-'.sanitize_title($font), $http.'://fonts.googleapis.com/css?family='.strip_tags($font));
					wp_enqueue_style('adamlabsgallery-google-font-'.sanitize_title($font));
				}
			}
		}
			...
			if(!empty($rules['ar-type'])){
				foreach($rules['ar-type'] as $key => $value){
					...
					switch($value){
						case 'meta':
							if($is_post){
								if(strpos($rules['ar-meta'][$key], 'adamlabsgallery-') === 0){
									if(!empty($m)){
										foreach($m as $me){
											if('adamlabsgallery-'.$me['handle'] == $rules['ar-meta'][$key]){
												$my_val = $meta->get_meta_value_by_handle($post['ID'],$rules['ar-meta'][$key]);
												break;
											}
										}
									}
								}elseif(strpos($rules['ar-meta'][$key], 'adamlabsgalleryl-') === 0){
									if(!empty($lm)){
										foreach($lm as $me){
											if('adamlabsgalleryl-'.$me['handle'] == $rules['ar-meta'][$key]){
						...
						default:
							if($this->item_media_type == $value){
								$my_val = apply_filters('adamlabsgallery_set_media_source', $my_val, $value, @$this->media_sources);
							}
						break;
					}
	...
	public function insert_layer($layer, $demo = false, $masonry = false, $grid_ids = '', $post_ids = ''){
		...
		if($hideunder > 0){
			...
			$hideunderClass = 'adamlabsgallery-handlehideunder ';
		}
		...
		if($masonry){
			...
			$transition = 'adamlabsgallery-'.esc_attr($base->getVar($layer['settings'], 'transition', 'fade')).esc_attr($base->getVar($layer['settings'], 'transition-type', ''));
			...
			if($meta_tran !== false && trim($meta_tran) !== '') $transition = ' adamlabsgallery-'.$meta_tran;
			
			if($transition == 'adamlabsgallery-none' || $transition == 'adamlabsgallery-noneout' || $base->getVar($layer['settings'], 'transition-type', '') == 'always'){ //no transition
				$transition = '';
				//$transition_split = '';
			}else{
			...
			if($transition) {
				...
				$transition = ' adamlabsgallery-transition';
		...
		switch($link_to){
							...
							if(!empty($this->lightbox_additions['items']) && $this->lightbox_additions['base'] == 'on') {
								...
								switch($order){
									...
									case 'adamlabsgallery':
										...
										$lb_class = ' adamlabsgallerybox adamlabsgallerybox-post';
										$lb_data = ' data-post="' . $post_ids . '"';
										$lb_data .= ' data-lbadamlabsgallery="' . $this->media_sources[$order] . '"';
										$lb_data .= ' data-gridid="' . $grid_ids . '" data-ispost="' . $is_post . '"';
			...
			break;
			case 'embedded_video':
				$video_play = ' adamlabsgallery-click-to-play-video';
			break;
			case 'ajax':
				$ajax_class = '';
				if(!empty($this->default_ajax_source_order)){ //only show if something is checked
			...
			case 'likepost':
				if(!empty($this->post['ID'])) 
					$text = '<a data-post_id="'.@$this->post['ID'].'" href="#"><span class="adamlabsgallery-post-like">'.$text.'</span></a>'; //javascript-link
				else $text = ''; 
			break;
		...
		if($is_post) {
			$post_class = (!isset($post['ID'])) ? '' : ' adamlabsgallery-post-'.$post['ID'];
		}
		else {
			$post_class = isset($this->post['post_id']) && !empty($this->post['post_id']) ? ' adamlabsgallery-post-'.$this->post['post_id'] : '';
		}
		...
		if($base->text_has_certain_tag($text, 'a') && !$do_ignore_styles){ //check if a tag exists, if yes, class will be set to a tags and not the wrapping div, also the div will receive the position and other stylings // && @$layer['settings']['source'] !== 'text'
			...
			/* 2.2 */
			// if(!empty($lb_class)) $text = str_replace('</a>', '<img class="adamlabsgallery-lb-dummy" src="'. ADAMLABS_GALLERY_PLUGIN_URL .'com/public/assets/images/300x200transparent.png"></a>', $text);
		...
		if($do_ignore_styles) $unique_class = 'adamlabsgallery-'.esc_attr($this->handle).'-nostyle-element-'.$layer['id'];
		...
		if($special_item == 'true' && $special_item_type == 'line-break'){ //line break element
			echo '              <div class="adamlabsgallery-'.$class.' '.$unique_class.' adamlabsgallery-none adamlabsgallery-clear" style="height: 5px; visibility: hidden;"></div>'."\n";
		}elseif(trim($text) !== ''){ //}elseif(!empty($text)){
			
			$use_tag = $base->getVar($layer['settings'], 'tag-type', 'div');
			echo '				<'.$use_tag.' class="adamlabsgallery-'.$class.$post_class.$video_play.$ajax_class.' '.$hideunderClass.$unique_class.$transition.'"'.$ajax_attr.$transition_split.$delay.$duration.$hideunderHTML;
	...
	public function get_post_value($handle, $separator, $function, $meta, $catmax = '-1', $taxonomy = ""){
		...
		switch($handle){
			...
			case 'excerpt':
				...
				if(empty($text)){
					$text = do_shortcode($base->strip_adamlabsgallery_shortcode($base->getVar($this->post, 'post_content')));
			...
			case 'likespost':
				$post_id = $base->getVar($this->post, 'ID', '');
				if(!empty($post_id)){
					$count = get_post_meta($post_id, "adamlabsgallery_votes_count", 0);
					if(!$count) $count[0] = 0;
					if(is_array($count)){
						$text = '<span class="adamlabsgallery-post-count">'.$count[0].'</span>';
					}
				}
				else{
					$text = '';
			...
			*/
			...
			default:
				$text = apply_filters('adamlabsgallery_post_meta_content', $text, $handle, $base->getVar($this->post, 'ID', ''), $this->post);
			break;
		}
	...
	public function get_woocommerce_value($meta, $separator, $catmax = false){
		...
		if(isset($this->post['ID'])){
			if(AdamLabsGallery_Woocommerce::is_woo_exists()){
				...
				if($adjustMax) {
					$text = AdamLabsGallery_Woocommerce::get_value_by_meta($this->post['ID'], $meta, $separator, $catmax);
				}
				else {
					$text = AdamLabsGallery_Woocommerce::get_value_by_meta($this->post['ID'], $meta, $separator);
				}
			}
		}
		return $text;
