...
class AdamLabsGallery_Meta {
	...
	public function add_new_meta($new_meta){
		...
		$new_meta = apply_filters('adamlabsgallery_add_new_meta', $new_meta);
		
		if(!isset($new_meta['handle']) || strlen($new_meta['handle']) < 3) return __('Wrong Handle received', ADAMLABS_GALLERY_TEXTDOMAIN);
		if(!isset($new_meta['name']) || strlen($new_meta['name']) < 3) return __('Wrong Name received', ADAMLABS_GALLERY_TEXTDOMAIN);
		if(!isset($new_meta['sort-type'])) $new_meta['sort-type'] = 'alphabetic';
		
		$metas = $this->get_all_meta(false);
		
		foreach($metas as $meta){
			if($meta['handle'] == $new_meta['handle']) return __('Meta with handle already exist, choose a different handle', ADAMLABS_GALLERY_TEXTDOMAIN);
		}
		...
		if($new_meta['type'] == 'select' || $new_meta['type'] == 'multi-select'){
			if(!isset($new_meta['sel']) || strlen($new_meta['sel']) < 3) return __('Wrong Select received', ADAMLABS_GALLERY_TEXTDOMAIN);
		...
		$do = update_option('adamlabsgallery-custom-meta', apply_filters('adamlabsgallery_add_new_meta_update', $metas));
	...
	public function edit_meta_by_handle($edit_meta){
		...
		$edit_meta = apply_filters('adamlabsgallery_edit_meta_by_handle', $edit_meta);
		
		if(!isset($edit_meta['handle']) || strlen($edit_meta['handle']) < 3) return __('Wrong Handle received', ADAMLABS_GALLERY_TEXTDOMAIN);
		if(!isset($edit_meta['name']) || strlen($edit_meta['name']) < 3) return __('Wrong Name received', ADAMLABS_GALLERY_TEXTDOMAIN);
		...
		foreach($metas as $key => $meta){
			if($meta['handle'] == $edit_meta['handle']){
				...
				$do = update_option('adamlabsgallery-custom-meta', apply_filters('adamlabsgallery_edit_meta_by_handle_update', $metas, $edit_meta));
				return true;
			}
		}
	...
	public function remove_meta_by_handle($handle){
		...
		$handle = apply_filters('adamlabsgallery_remove_meta_by_handle', $handle);
		...
		foreach($metas as $key => $meta){
			if($meta['handle'] == $handle){
				...
				$do = update_option('adamlabsgallery-custom-meta', apply_filters('adamlabsgallery_remove_meta_by_handle_update', $metas));
				return true;
			}
		}
		
		return __('Meta not found! Wrong handle given.', ADAMLABS_GALLERY_TEXTDOMAIN);
	}
	...
	public function get_all_meta($links = true){
		...
		$meta = get_option('adamlabsgallery-custom-meta', array());
		...
		return apply_filters('adamlabsgallery_get_all_meta', $meta, $links);
	}
	...
	public function get_all_meta_handle(){
		...
		if(!empty($meta)){
			foreach($meta as $m){
				$metas[] = 'adamlabsgallery-'.$m['handle'];
			}
		}
		
		if(AdamLabsGallery_Woocommerce::is_woo_exists()){
			$meta = AdamLabsGallery_Woocommerce::get_meta_array();
		...
		return apply_filters('adamlabsgallery_get_all_meta_handle', $metas);
	}
	...
	public function prepare_select_by_string($string){
		...
		return apply_filters('adamlabsgallery_prepare_select_by_string', explode(',', $string), $string);
	...
	public function get_meta_value_by_handle($post_id, $handle){
		...
		if(!empty($cmeta)){
			foreach($cmeta as $me){
				if('adamlabsgallery-'.$me['handle'] == $handle){
					if($me['type'] == 'image'){
						if(intval($text) > 0){
							//get URL to Image
		...
		//check woocommerce
		if(AdamLabsGallery_Woocommerce::is_woo_exists()){
			$wc_text = AdamLabsGallery_Woocommerce::get_value_by_meta($post_id, $handle);
		...
		return apply_filters('adamlabsgallery_get_meta_value_by_handle', $text, $post_id, $handle);
	}
	...
	public function replace_all_meta_in_text($post_id, $text){
			...
			foreach($arr_matches as $matches){
				if(is_array($matches)){
					foreach($matches as $match){
						...
						if(!empty($cmeta)){
							foreach($cmeta as $me){
								if('adamlabsgallery-'.$me['handle'] == $meta){
									if($me['type'] == 'image'){
						...
						//check woocommerce
						if(AdamLabsGallery_Woocommerce::is_woo_exists()){
							$wc_text = AdamLabsGallery_Woocommerce::get_value_by_meta($post_id, $meta);
						...
						if(empty($meta_value) && !empty($my_post)){ //try to get from post
							switch($meta){
								...
								case 'likespost':
									if(!empty($my_post['ID'])){
										$count = get_post_meta($my_post['ID'], "adamlabsgallery_votes_count", '');
										$count[0] = isset($count[0]) ? $count[0] : 0;
										$meta_value = '<span class="adamlabsgallery-post-count">'.$count[0].'</span>';
									}
									else{
										$meta_value = '';
									}
								...
								case 'taxonomy':
									...
									break;
								default:
									$meta_value = apply_filters('adamlabsgallery_post_meta_content', $meta_value, $meta, $my_post['ID'], $my_post);
									break;
								break;
							}
		...
		return apply_filters('adamlabsgallery_replace_all_meta_in_text', $text, $post_id, $arr_matches);
	}
	...
	public function replace_all_custom_element_meta_in_text($values, $text){
		...
		return apply_filters('adamlabsgallery_replace_all_custom_element_meta_in_text', $text, $values, $arr_matches);
	}
	...
	public function get_post_video_ratios($post_id){
		...
		$ratio['vimeo'] = get_post_meta($post_id, 'adamlabsgallery_vimeo_ratio', true);
		$ratio['youtube'] = get_post_meta($post_id, 'adamlabsgallery_youtube_ratio', true);
		$ratio['wistia'] = get_post_meta($post_id, 'adamlabsgallery_wistia_ratio', true);
		$ratio['html5'] = get_post_meta($post_id, 'adamlabsgallery_html5_ratio', true);
		$ratio['soundcloud'] = get_post_meta($post_id, 'adamlabsgallery_soundcloud_ratio', true);
		
		return apply_filters('adamlabsgallery_get_post_video_ratios', $ratio, $post_id);
	}
	...
	public function get_custom_video_ratios($values){
		...
		return apply_filters('adamlabsgallery_get_custom_video_ratios', $ratio, $values);
	}
...
/**
 * AdamLabsGallery_Meta_Linking
 **/
class AdamLabsGallery_Meta_Linking {
	...
	public function add_new_link_meta($new_meta){
		...
		if(!isset($new_meta['handle']) || strlen($new_meta['handle']) < 3) return __('Wrong Handle received', ADAMLABS_GALLERY_TEXTDOMAIN);
		if(!isset($new_meta['name']) || strlen($new_meta['name']) < 3) return __('Wrong Name received', ADAMLABS_GALLERY_TEXTDOMAIN);
		if(!isset($new_meta['original']) || strlen($new_meta['original']) < 3) return __('Wrong Linking received', ADAMLABS_GALLERY_TEXTDOMAIN);
		...
		$do = update_option('adamlabsgallery-custom-link-meta', apply_filters('adamlabsgallery_add_new_link_meta', $metas, $new_meta, $new));
	...
	public function edit_link_meta_by_handle($edit_meta){
		...
		if(!isset($edit_meta['handle']) || strlen($edit_meta['handle']) < 3) return __('Wrong Handle received', ADAMLABS_GALLERY_TEXTDOMAIN);
		if(!isset($edit_meta['name']) || strlen($edit_meta['name']) < 3) return __('Wrong Name received', ADAMLABS_GALLERY_TEXTDOMAIN);
		if(!isset($edit_meta['original']) || strlen($edit_meta['original']) < 3) return __('Wrong Linking received', ADAMLABS_GALLERY_TEXTDOMAIN);
		...
		foreach($metas as $key => $meta){
			if($meta['handle'] == $edit_meta['handle']){
				...
				$do = update_option('adamlabsgallery-custom-link-meta', apply_filters('adamlabsgallery_edit_link_meta_by_handle', $metas, $edit_meta, $before));
				return true;
			}
		}
	...
	public function remove_link_meta_by_handle($handle){
		...
		foreach($metas as $key => $meta){
			if($meta['handle'] == $handle){
				$before = $metas[$key];
				unset($metas[$key]);
				$do = update_option('adamlabsgallery-custom-link-meta', apply_filters('adamlabsgallery_edit_link_meta_by_handle', $metas, $handle, $before));
				return true;
			}
		}
	...
	public function get_all_link_meta(){
		...
		$meta = get_option('adamlabsgallery-custom-link-meta', array());
		
		return apply_filters('adamlabsgallery_get_all_link_meta', $meta);
	}
	...
	public function get_all_link_meta_handle(){
		...
		if(!empty($meta)){
			foreach($meta as $m){
				$metas[] = 'adamlabsgalleryl-'.$m['handle'];
			}
		}
		
		return apply_filters('adamlabsgallery_get_all_link_meta_handle', $metas);
	}
	...
	public function get_link_meta_value_by_handle($post_id, $handle){
		...
		if(!empty($metas)){
			foreach($metas as $m){
				if($handle == 'adamlabsgalleryl-'.$m['handle']){
					$orig = $m['original'];
					break;
				}
			}
		...
		return apply_filters('adamlabsgallery_get_link_meta_value_by_handle', $text, $post_id, $handle);
	}
