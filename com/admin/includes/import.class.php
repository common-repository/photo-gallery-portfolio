...
class AdamLabsGallery_Import {
	...
	public function import_grids($import_grids, $import_ids = true, $check_append = true){
		$d = apply_filters('adamlabsgallery_import_grids', array('import_grids' => $import_grids, 'import_ids' => $import_ids, 'check_append' => $check_append));
					...
					if(!empty($skin)){
						...
						if(isset($i_grid['layers']) && !empty($i_grid['layers'])) {
							...
							foreach($mods as $key => $val) {
								...
								if(isset($mod['adamlabsgallery_settings_custom_meta_skin']) && !empty($mod['adamlabsgallery_settings_custom_meta_skin'])) {
									
									$skin_mods = $mod['adamlabsgallery_settings_custom_meta_skin'];
									$mod_ids = array();
									foreach($skin_mods as $i) {
										$mod_ids[] = $skin['id'];
									}
									$mod['adamlabsgallery_settings_custom_meta_skin'] = $mod_ids;
								...
								if(isset($mod['custom-type']) && !empty($mod['custom-type'])) {
									...
									if($alternateSkin === 'blank') {
										...
										if(empty($blankSkinId)) {
											$blankSkin = $item_skin->get_id_by_handle('adamlabsgalleryblankskin');
	...
	public function import_skins($import_skins, $import_ids, $check_append = true){
		$d = apply_filters('adamlabsgallery_import_skins', array('import_skins' => $import_skins, 'import_ids' => $import_ids, 'check_append' => $check_append));
		...
		if($import_skins !== false && !empty($import_skins)){
			...
			$skins = $item_skin->get_adamlabsgallery_item_skins('all', false); //false = do not decode params
	...
	public function import_elements($import_elements, $import_ids, $check_append = true){
		$d = apply_filters('adamlabsgallery_import_elements', array('import_elements' => $import_elements, 'import_ids' => $import_ids, 'check_append' => $check_append));
		...
		if($import_elements !== false && !empty($import_elements)){
			global $wpdb;
			$c_elements = new AdamLabsGallery_Item_Element();
			$base = new AdamLabsGallery_Base();
			$table_name = $wpdb->prefix . AdamLabsGallery::TABLE_ITEM_ELEMENTS;
		
			$elements = $c_elements->get_adamlabsgallery_item_elements();
	...
	public function import_navigation_skins($import_navigation_skins, $import_ids, $check_append = true){
		$d = apply_filters('adamlabsgallery_import_navigation_skins', array('import_navigation_skins' => $import_navigation_skins, 'import_ids' => $import_ids, 'check_append' => $check_append));
		...
		if($import_navigation_skins !== false && !empty($import_navigation_skins)){
			global $wpdb;
			$c_nav = new AdamLabsGallery_Navigation();
			$base = new AdamLabsGallery_Base();
			$table_name = $wpdb->prefix . AdamLabsGallery::TABLE_NAVIGATION_SKINS;
		
			$nav_skins = $c_nav->get_adamlabsgallery_navigation_skins();
			...
			foreach($import_navigation_skins as $i_nav_skin){
				...
				$i_nav_skin['css'] = preg_replace_callback('/\\\\u([0-9a-fA-F]{4})/', array('AdamLabsGallery_Import', 'mb_convert_string'), $i_nav_skin['css']);
	...
	public function import_custom_meta($import_custom_meta, $import_handles = true, $check_append = true){
		$d = apply_filters('adamlabsgallery_import_custom_meta', array('import_custom_meta' => $import_custom_meta, 'import_handles' => $import_handles, 'check_append' => $check_append));
		...
		$link_metas = new AdamLabsGallery_Meta_Linking();
	...
	public function import_adamlabs_fonts($import_adamlabs_fonts, $import_handles = true, $check_append = true){
		$d = apply_filters('adamlabsgallery_import_adamlabs_fonts', array('import_adamlabs_fonts' => $import_adamlabs_fonts, 'import_handles' => $import_handles, 'check_append' => $check_append));
		$import_adamlabs_fonts = $d['import_adamlabs_fonts'];
		$import_handles = $d['import_handles'];
		$check_append = $d['check_append'];

		$base = new AdamLabsGallery_Base();
		$fonts = new AdamLabs_Fonts();
		$adamlabs_fonts = $fonts->get_all_fonts();
		
		foreach($import_adamlabs_fonts as $i_adamlabs_font){
			if(!empty($import_handles) && is_array($import_handles)){
				$found = false;
				foreach($import_handles as $handle){
					if($i_adamlabs_font['handle'] == $handle){
						$found = true;
						break;
					}
				}
			...
			$exist = false;
			if(!empty($adamlabs_fonts)){
				foreach($adamlabs_fonts as $font){
					if($font['handle'] == $i_adamlabs_font['handle']){
						$exist = true;
						break;
					}
				}
			...
			if($exist){ //skin exists - append or overwrite
				if($check_append){ //check in $_POST if append or overwrite
					$do = $base->getVar($this->overwrite_data, 'adamlabs-fonts-overwrite-'.$i_adamlabs_font['handle'], 'append');
					$append = ($do == 'append') ? true : false;
				}
			}
			...
			if($import_handles !== true){
				if($append){ //append
					if($exist){
						$orig_handle = $i_adamlabs_font['handle'];
						$i_adamlabs_font['handle'] = $i_adamlabs_font['handle'].'-'.date('His');
					}
					$fonts->add_new_font($i_adamlabs_font);
				}else{ //overwrite
					$fonts->edit_font_by_handle($i_adamlabs_font);
				}
			}else{ //create or overwrite
				if($exist){
					$fonts->edit_font_by_handle($i_adamlabs_font);
	...
	public function import_global_styles($import_global_styles, $check_append = true){
		$d = apply_filters('adamlabsgallery_import_global_styles', array('import_global_styles' => $import_global_styles, 'import_handles' => true, 'check_append' => $check_append));
		$import_global_styles = $d['import_global_styles'];
		$check_append = $d['check_append'];
		
		$base = new AdamLabsGallery_Base();
		$c_css = new AdamLabsGallery_Global_Css();
		...
		$import_global_styles = preg_replace_callback('/\\\\u([0-9a-fA-F]{4})/', array('AdamLabsGallery_Import', 'mb_convert_string'), $import_global_styles);
